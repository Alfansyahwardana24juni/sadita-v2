@php
    ...
    $isOrder = request()->routeIs('checkout') || request()->routeIs('checkout.*') || request()->routeIs('cart.*') || request()->routeIs('toko.orders');
@endphp
...
<nav class="relative z-50 flex h-[64px] w-full items-center justify-between rounded-t-3xl sm:rounded-b-3xl bg-white px-2 shadow-[0_-8px_30px_rgba(0,0,0,0.06)] sm:shadow-[0_8px_30px_rgba(0,0,0,0.08)] border-t border-slate-100 sm:border">
    <div class="flex flex-1 items-center justify-around">
        ...
        <a href="{{ route('toko.home') }}" class="group flex flex-col items-center gap-1 p-2 transition-colors active:scale-95">
            <span class="material-symbols-outlined text-[24px] {{ $isTokoHome ? 'text-[#800000]' : 'text-slate-400 group-hover:text-slate-600' }}" @if($isTokoHome) style="font-variation-settings:'FILL' 1" @endif>store</span>
            <span class="text-[9px] font-semibold {{ $isTokoHome ? 'text-[#800000]' : 'text-slate-400 group-hover:text-slate-600' }}">{{ __('Toko') }}</span>
        </a>
    </div>

    <!-- Tombol Tengah (Katalog) dengan efek Cutout -->
    <div class="relative flex w-[64px] justify-center shrink-0">
        <div class="absolute -top-[42px] flex h-[60px] w-[60px] items-center justify-center rounded-full bg-white shadow-[0_-6px_15px_rgba(0,0,0,0.04)]">
            <a href="{{ route('toko.katalog') }}" class="flex h-[50px] w-[50px] flex-col items-center justify-center rounded-full transition-transform hover:scale-105 active:scale-95 {{ $isKatalog ? 'bg-[#800000] text-white shadow-lg shadow-[#800000]/30' : 'bg-slate-100 text-slate-500 hover:bg-slate-200' }}">
                <span class="material-symbols-outlined text-[24px]" @if($isKatalog) style="font-variation-settings:'FILL' 1" @endif>grid_view</span>
            </a>
        </div>
    </div>
    ...
    <div class="flex flex-1 items-center justify-around">
        <a href="{{ route('toko.orders') }}" class="group flex flex-col items-center gap-1 p-2 transition-colors active:scale-95">
            <span class="material-symbols-outlined text-[24px] {{ $isOrder ? 'text-[#800000]' : 'text-slate-400 group-hover:text-slate-600' }}" @if($isOrder) style="font-variation-settings:'FILL' 1" @endif>receipt_long</span>
            <span class="text-[9px] font-semibold {{ $isOrder ? 'text-[#800000]' : 'text-slate-400 group-hover:text-slate-600' }}">{{ __('Order') }}</span>
        </a>
