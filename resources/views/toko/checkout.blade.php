<x-layouts.toko title="Checkout - SADITA">
    ...
    <div class="bg-surface min-h-screen pb-28">
        <div class="px-4 py-5 md:py-6">
            ...
            <form id="checkout-form" action="{{ route('checkout.store') }}" method="POST" class="flex flex-col gap-4">
                @csrf
                <input type="hidden" name="selected_items_str" value="{{ old('selected_items_str', $selectedItemsStr ?? '') }}">
                ...
                <div class="bg-white rounded-2xl border border-line p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.04)]">
                    ...
                    <div class="flex flex-col gap-3.5">
                        ...
                        <div>
                            <label class="mb-1 block text-xs font-bold uppercase tracking-wide text-muted">Provinsi *</label>
                            <select name="customer_province" id="province-select" required
                                class="h-11 w-full rounded-xl border border-line bg-surface/50 px-3 text-sm focus:bg-white focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all font-medium @error('customer_province') border-red-500 @enderror">
                        ...
                        <div id="city-container" style="display:none;">
                            <label class="mb-1 block text-xs font-bold uppercase tracking-wide text-muted">Kota/Kabupaten *</label>
                            <select name="customer_regency_id" id="city-select" required
                                class="h-11 w-full rounded-xl border border-line bg-surface/50 px-3 text-sm focus:bg-white focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all font-medium">
                        ...
                        <div id="district-container" style="display:none;">
                            <label class="mb-1 block text-xs font-bold uppercase tracking-wide text-muted">Kecamatan *</label>
                        ...
                        <div id="village-container" style="display:none;">
                            <label class="mb-1 block text-xs font-bold uppercase tracking-wide text-muted">Kelurahan/Desa *</label>
                            <select name="village_id" id="village-select" required
                                class="h-11 w-full rounded-xl border border-line bg-surface/50 px-3 text-sm focus:bg-white focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all font-medium">
                ...
                {{-- 3. Metode Pengiriman --}}
                <div class="bg-white rounded-2xl border border-line p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.04)]" id="shipping-section-wrapper" style="display:none;">
                    <div class="flex items-center gap-3 mb-4 pb-3 border-b border-line border-dashed">
                        <div class="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 shrink-0">
                ...
                <div class="bg-white rounded-2xl border border-line p-5 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.04)]">
                    <h2 class="text-xs font-black text-ink mb-3 uppercase tracking-wider flex items-center gap-2">
                        <span class="material-symbols-outlined text-primary text-[16px]">sell</span>
                        Promo & Voucher
                    </h2>
                    <div class="flex gap-2">
                        <input type="text" name="voucher_code" id="voucher-code-input" value="{{ old('voucher_code') }}"
                            class="h-11 flex-1 rounded-xl border border-line bg-surface/50 px-3 text-sm font-bold placeholder-muted focus:bg-white focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/10 transition-all uppercase"
                            placeholder="KODE PROMO" />
                        <button type="button" id="apply-voucher-button"
                            class="h-11 px-4 rounded-xl bg-ink text-sm font-bold text-white hover:bg-ink/90 active:scale-95 transition-all shadow-md">
                            Terapkan
                        </button>
                    </div>
                    <p id="voucher-message" class="text-[11px] mt-2 hidden font-semibold"></p>
                </div>
                    ...
                    <div class="space-y-2.5">
                        <label class="group flex flex-col gap-2 rounded-xl border-2 border-line bg-white p-3 cursor-pointer hover:border-primary/40 transition-all has-[:checked]:border-primary has-[:checked]:bg-primary/5 has-[:checked]:shadow-[0_4px_12px_-4px_rgba(var(--color-primary),0.1)]">
                            <div class="flex items-center gap-3">
                                <input type="radio" name="payment_method" value="transfer" @checked(old('payment_method', 'transfer') === 'transfer') required class="text-primary focus:ring-primary/20 accent-primary w-4 h-4">
                                <div class="flex-1">
                                    <p class="font-bold text-sm text-ink flex items-center gap-1.5">🏦 Transfer Bank</p>
                            ...
                            <div class="hidden group-has-[:checked]:block mt-1 pl-7">
                                <div class="p-2 rounded-lg bg-white border border-line text-[11px] leading-5 text-muted">
                                    ...
                                    <strong>Bank: {{ config('sadita.bank_name', 'BCA') }}</strong><br>
                                    <strong>No. Rek: {{ config('sadita.bank_account', '-') }}</strong><br>
                                    <strong>A/N: {{ config('sadita.bank_holder', '-') }}</strong>
                                </div>
                            </div>
                        </label>
                        ...
                        <label class="group flex flex-col gap-2 rounded-xl border-2 border-line bg-white p-3 cursor-pointer hover:border-primary/40 transition-all has-[:checked]:border-primary has-[:checked]:bg-primary/5 has-[:checked]:shadow-[0_4px_12px_-4px_rgba(var(--color-primary),0.1)]">
                            <div class="flex items-center gap-3">
                                <input type="radio" name="payment_method" value="qris" @checked(old('payment_method') === 'qris') required class="text-primary focus:ring-primary/20 accent-primary w-4 h-4">
                                <div class="flex-1">
                ...
                <div class="bg-white rounded-2xl border border-line p-5 shadow-[0_4px_20px_-8px_rgba(0,0,0,0.1)] relative overflow-hidden">
                    ...
                    <div class="space-y-2.5 text-sm font-medium text-muted mb-4 pb-4 border-b border-line border-dashed relative z-10">
                        ...
                        <div class="flex justify-between items-center">
                            <span>Diskon</span>
                            <span class="font-bold text-moss" id="detail-discount">-Rp 0</span>
                        </div>
                    </div>
    ...
    <script>
        (() => {
            ...
            const voucherBtn                = document.getElementById('apply-voucher-button');
            ...
            // ── Voucher interactive trigger ───────────────────────────────────
            let currentDiscount = 0;

            if (voucherBtn && voucherInput && voucherMsg) {
                voucherBtn.addEventListener('click', async () => {
                    const code = voucherInput.value.trim();
                    if (!code) {
                        voucherMsg.textContent = 'Masukkan kode voucher terlebih dahulu.';
                        voucherMsg.className = 'text-xs mt-2 text-red-500 block animate-fade-in';
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            voucherMsg.className = 'text-xs mt-2 text-moss font-bold block animate-fade-in';
                            voucherMsg.classList.remove('hidden');
                            currentDiscount = data.discount || 0;
                        } else {
                            voucherMsg.textContent = data.message || 'Voucher tidak valid.';
                            voucherMsg.className = 'text-xs mt-2 text-red-500 block animate-fade-in';
                            voucherMsg.classList.remove('hidden');
                            currentDiscount = 0;
                        }
                    } catch (e) {
                        voucherMsg.textContent = 'Terjadi kesalahan saat mengecek voucher.';
                        voucherMsg.className = 'text-xs mt-2 text-red-500 block animate-fade-in';
                        voucherMsg.classList.remove('hidden');
                        currentDiscount = 0;
                    }
            ...
            function hideShippingSection() {
                shippingSectionWrapper.style.display = 'none';
            ...
            function updateTotal() {
                const total = Math.max(0, subtotal + selectedShippingCost - currentDiscount);
                ...
                if (detailDiscount) detailDiscount.textContent = currentDiscount === 0 ? '-Rp 0' : '-Rp ' + currentDiscount.toLocaleString('id-ID');
